...
public class Main {
	...
	private static class Student {
		...
		public Student(int no, int x, int y) {
			this.no = no;
			this.x = x;
			this.y = y;
		}
	...
	public static void main(String[] args) throws IOException{
		 ...
		 * 입력 다 받아놓고 시작.
		 * 3 ≤ N ≤ 20, 
		 * N2개의 줄에 학생의 번호와 그 학생이 좋아하는 학생 4명의 번호
		 * 입력으로 주어지는 학생의 번호, 좋아하는 학생의 번호는 N^2보다 작거나 같은 자연수이다 -> 1부터 순서대로
		 * 진행하면서 현재 학생이 앉은 자리, 좋아하는 학생 몇명이랑 인접해있는지 저장.
		 * 1. 학생 클래스 : 앉은자리 x, y, 인접한 수
		 * Student[] 배열 만들어서 번호를 index로 사용해서 저장.
		 */
		
		/*
		 * 문제풀다가 필요할 것 같아서 추가한 부분
		 * 좋아하는 학생과 가장 많이 인접한 칸 찾기. -> Point 클래스 이용.
		 * List에 넣어서 count 세기. 해봤자 4*4 16개 나옴.
		 * 그러고나서 list를 정렬. 정렬 어려웠음 ㅜㅜ
		 * 
		 * 좋아한다는게 진짜 짝사랑일때도 있음..
		 * 처음에 양방향인지 알고 무조건 올려줘서 왜 틀린거지? 하고 있었다.
		 */
...
//			}
			...
			students[curStudent] = new Student(curStudent, curseat.x, curseat.y);
