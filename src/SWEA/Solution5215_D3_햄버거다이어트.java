...
public class Solution5215_D3_햄버거다이어트 {
...
//			});
	...
	private static void subset(int ssum, int ksum, int index) {
		if(ksum >= L) return;
		if(ssum > result) result = ssum;
		if(index >= N) return;

		subset(ssum+table[index][0], ksum+table[index][1], index+1);
		subset(ssum, ksum, index+1);
	}
